between_diss <- function (D, A, A_c, w = rep(1. / nrow(Z), nrow(Z))) 
{ 
  # checks there is not a empty class
  if (is.null(A) | is.null(A_c)) stop('between_cluster_inert : one of the two cluster is empty!')
  inert <- inertdiss(D,indices=c(A,A_c),wt=w) - inertdiss(D,indices=A,wt=w) - inertdiss(D,indices=A_c,wt=w)
  return(inert)
}
...
#' data(gironde)
#' n <- nrow(gironde$dat)
#' Z <- scale(gironde$dat)*sqrt(n/(n-1))
#' inertdiss(dist(Z)) # pseudo inertia
#' inert(Z) #equals for euclidean distance
#' 
#' w <- gironde$map@data$POPULATION # non uniform weights 
